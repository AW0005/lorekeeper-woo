            ...
            if (isset($data['character_currency_id'][$data['character_id']]) && isset($data['character_quantity'][$data['character_id']])) {
                ...
                foreach ($data['character_rewardable_id'][$data['character_id']] as $key => $reward) {
                    switch ($data['character_rewardable_type'][$data['character_id']][$key]) {
                        case 'Currency':
                            if ($data['character_rewardable_quantity'][$data['character_id']][$key]) addAsset($assets, $data['currencies'][$reward], $data['character_rewardable_quantity'][$data['character_id']][$key]);
                            break;
                        case 'Item':
                            if ($data['character_rewardable_quantity'][$data['character_id']][$key]) addAsset($assets, $data['items'][$reward], $data['character_rewardable_quantity'][$data['character_id']][$key]);
                            break;
                        case 'LootTable':
                            if ($data['character_rewardable_quantity'][$data['character_id']][$key]) addAsset($assets, $data['tables'][$reward], $data['character_rewardable_quantity'][$data['character_id']][$key]);
                            break;
                    }
                }
            }
    ...
    public function approveSubmission($data, $user) {
        ...
        try {
            ...
            $logEvent = LogEvent::create([
                'event_type' => $submission->prompt_id ? 'Prompt Approved' : 'Claim Approved',
            ]);
            ...
            if (isset($addonData['user_items'])) {
                ...
                foreach ($stacks as $stackId => $quantity) {
                    ...
                    if (!$inventoryManager->debitStack($user, $logEvent, ['data' => 'Item used in submission (<a href="' . $submission->viewUrl . '">#' . $submission->id . '</a>)'], $stack, $quantity)) throw new \Exception("Failed to create log for item stack.");
                }
            ...
            if (!$rewards = fillUserAssets($rewards, $user, $submission->user, $logEvent, $promptData)) throw new \Exception("Failed to distribute rewards to user.");
            ...
            foreach ($characters as $c) {
                ...
                if (!$assets = fillCharacterAssets($assets, $user, $c, $logEvent, $promptData, $submission->user)) throw new \Exception("Failed to distribute rewards to character.");
