        ...
        try {
            ...
            if ($senderStacks) {
                foreach ($senderStacks as $stack) {
                    ...
                    $inventoryManager->moveStack($trade->sender, $trade->recipient, $logEvent, ['data' => 'Received in trade [<a href="' . $trade->url . '">#' . $trade->id . '</a>]'], $stack, $quantity);
            ...
            if ($recipientStacks) {
                foreach ($recipientStacks as $stack) {
                    ...
                    $inventoryManager->moveStack($trade->recipient, $trade->sender, $logEvent, ['data' => 'Received in trade [<a href="' . $trade->url . '">#' . $trade->id . '</a>]'], $stack, $quantity);
                    $userItemRow = UserItem::find($stack->id);
                    if (!$userItemRow) throw new \Exception("Cannot return an invalid item. (" . $userItemId . ")");
                    if ($userItemRow->trade_count < $quantity) throw new \Exception("Cannot return more items than was held. (" . $userItemId . ")");
                    $userItemRow->trade_count -= $quantity;
            ...
            foreach ($senderCharacters as $character) $characterManager->moveCharacter($character, $trade->recipient, 'Transferred in Trade [<a href="' . $trade->url . '">#' . $trade->id . '</a>]', isset($cooldowns[$character->id]) ? $cooldowns[$character->id] : $defaultCooldown, $logEvent);

            foreach ($recipientCharacters as $character) $characterManager->moveCharacter($character, $trade->sender, 'Transferred in Trade [<a href="' . $trade->url . '">#' . $trade->id . '</a>]', isset($cooldowns[$character->id]) ? $cooldowns[$character->id] : $defaultCooldown, $logEvent);
            ...
            foreach (['sender', 'recipient'] as $type) {
                ...
                if (isset($tradeData[$type]['currencies'])) {
                    foreach ($tradeData[$type]['currencies'] as $currencyId => $quantity) {
                        ...
                        if (!$currencyManager->creditCurrency($trade->{$type}, $trade->{$recipientType}, $logEvent, 'Received in trade [<a href="' . $trade->url . '">#' . $trade->id . ']', $currency, $quantity)) throw new \Exception("Could not credit currency. (" . $currencyId . ")");
                    }
                }
            }
