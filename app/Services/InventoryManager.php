    ...
    */
        ...
        try {
            ...
            foreach ($users as $user) {
                foreach ($items as $item) {
                    if ($this->creditItem($staff, $user, $logEvent, Arr::only($data, ['data', 'disallow_transfer', 'notes']), $item, $keyed_quantities[$item->id])) {
    ...
    public function grantCharacterItems($data, $character, $staff) {
        ...
        try {
            ...
            foreach ($items as $item) {
                $this->creditItem($staff, $character, $logEvent, Arr::only($data, ['data', 'disallow_transfer', 'notes']), $item, $keyed_quantities[$item->id]);
    ...
    public function transferCharacterStack($sender, $recipient, $stacks, $quantities) {
        ...
        try {
            ...
            foreach ($stacks as $key => $stack) {
                ...
                $this->creditItem($sender, $recipient, $logEvent, $stack->data, $stack->item, $quantity);
    ...
    public function resellStack($user, $stacks, $quantities) {
        ...
        try {
            foreach ($stacks as $key => $stack) {
                ...
                $logEvent = LogEvent::create([
                    'event_type' => $stack->user_id == $user->id ? 'Item Sold by User' : 'Item Sold by Staff',
                ]);
                ...
                if (isset($stack->item->data['resell']) && $stack->item->data['resell']) {
                    ...
                    if (!$currencyManager->creditCurrency(null, $oldUser, $logEvent, 'Sold ' . $stack->item->displayName . ' ×' . $quantity, $currency, $currencyQuantity)) {
                        throw new \Exception("Failed to credit currency.");
                    }
                }


                if ($this->debitStack($stack->user, $logEvent, ['data' => ($stack->user_id != $user->id ? 'Sold by ' . $user->displayName : '')], $stack, $quantity)) {
                    if ($stack->user_id != $user->id)
                        Notifications::create('ITEM_REMOVAL', $oldUser, [
                            'item_name' => $stack->item->name,
                            'item_quantity' => $quantity,
