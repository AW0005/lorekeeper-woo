        ...
        try {
            ...
            if (!isset($data['ineligible'])) {
                ...
                $logEvent = LogEvent::create([
                    'event_type' => 'Gallery Submission Reward',
                ]);

                // Then cycle through associated users and award currency
                if (isset($data['value']['submitted'])) {
                    if (!$currencyManager->creditCurrency($user, $submission->user, $logEvent, $awardData, $currency, $data['value']['submitted'][$submission->user->id])) throw new \Exception("Failed to award currency to submitting user.");
                ...
                if (isset($data['value']['collaborator'])) {
                    foreach ($submission->collaborators as $collaborator) {
                        if ($data['value']['collaborator'][$collaborator->user->id] > 0) {
                            ...
                            if (!$currencyManager->creditCurrency($user, $collaborator->user, $logEvent, $awardData, $currency, $data['value']['collaborator'][$collaborator->user->id])) throw new \Exception("Failed to award currency to one or more collaborators.");
                ...
                if (isset($data['value']['participant'])) {
                    foreach ($submission->participants as $participant) {
                        if ($data['value']['participant'][$participant->user->id] > 0) {
                            if (!$currencyManager->creditCurrency($user, $participant->user, $logEvent, $awardData, $currency, $data['value']['participant'][$participant->user->id])) throw new \Exception("Failed to award currency to one or more participants.");
