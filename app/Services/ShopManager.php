    ...
    public function buyStock($data, $user) {
        ...
        try {
            ...
            if ($data['bank'] == 'character') {
                ...
                if (!(new CurrencyManager)->debitCurrency($character, null, $logEvent, 'Purchased ' . $shopStock->item->name . ' from ' . $shop->name, $shopStock->currency, $total_cost)) throw new \Exception("Not enough currency to make this purchase.");
            } else {
                ...
                if ($shopStock->cost > 0 && !(new CurrencyManager)->debitCurrency($user, null, $logEvent, 'Purchased ' . $shopStock->item->name . ' from ' . $shop->name, $shopStock->currency, $total_cost)) throw new \Exception("Not enough currency to make this purchase.");
            }
            ...
            if (!(new InventoryManager)->creditItem(null, $user, $logEvent, [
