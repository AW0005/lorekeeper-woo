...
class CharacterImageController extends Controller
{
    ...
    */
    ...
    public function postNewImage(Request $request, CharacterManager $service, $slug)
    {
        ...
        $data = $request->only(['image', 'thumbnail', 'artist_url', 'artist_id', 'artist_type', 'designer_url', 'designer_id', 'designer_type','species_id', 'subtype_id', 'rarity_id', 'feature_id', 'feature_data','set_active', 'is_valid', 'is_visible']);
        $this->character = Character::where('slug', $slug)->first();
        if(!$this->character) abort(404);
    ...
    {
        $data = $request->only(['species_id', 'subtype_id', 'rarity_id', 'feature_id', 'feature_data', 'is_android']);
        $image = CharacterImage::find($id);
        if(!$image) abort(404);
    ...
    {
        $data = $request->only(['image', 'thumbnail']);
        $image = CharacterImage::find($id);
        if(!$image) abort(404);
