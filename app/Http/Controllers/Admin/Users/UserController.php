...
class UserController extends Controller
{
    ...
    public function postUserBasicInfo(Request $request, $name)
    {
        ...
        elseif (!Auth::user()->canEditRank($user->rank)) {
            ...
            if(!(new UserService)->logAdminAction(Auth::user(), 'Edited User', 'Edited '.$user->displayname)) { flash('Failed to log admin action.')->error(); return redirect()->back();}
    ...
    public function postUserBirthday(Request $request, $name)
    {
        ...
        if(!(new UserService)->logAdminAction(Auth::user(), 'Edited User', 'Edited '.$user->displayname.' birthday')) { flash('Failed to log admin action.')->error(); return redirect()->back();}
