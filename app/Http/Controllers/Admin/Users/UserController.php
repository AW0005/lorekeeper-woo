...
class UserController extends Controller
{
    ...
    public function postUserBasicInfo(Request $request, $name)
    {
        ...
        elseif (!Auth::user()->canEditRank($user->rank)) {
            ...
            if(!(new UserService)->logAdminAction($staff, 'Edited User', 'Edited '.$user->displayname)) { flash('Failed to log admin action.')->error(); return redirect()->back();}
    ...
    public function postUserAlias(Request $request, $name, $id)
    {
        ...
        if(!$user) flash('Invalid user.')->error();
            ...
            if(!(new UserService)->logAdminAction($staff, 'Edited User', 'Cleared '.$user->displayname.'\' alias')) { flash('Failed to log admin action.')->error(); return redirect()->back(); }
    ...
    public function postUserBirthday(Request $request, $name)
    {
        ...
        if(!(new UserService)->logAdminAction($staff, 'Edited User', 'Edited '.$user->displayname.' birthday')) { flash('Failed to log admin action.')->error(); return redirect()->back();}
